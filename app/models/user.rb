...
#                          email:auth.extra.user_hash.email, #色々頑張りましたがtwitterではemail取得できません
  ...
  def twitter_client(user)
    Twitter::REST::Client.new do |config|
        ...
        config.access_token        = user.try(:access_token) || ENV["TWITTER_ACCESS_TOKEN"]
        config.access_token_secret = user.try(:access_token_secret) || ENV["TWITTER_ACCESS_TOKEN_SECRET"]
    end
  end
